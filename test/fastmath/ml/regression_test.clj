...
(defn lm-tests
  ...
  ([ys xss options lm-r]
   (let [lm (sut/lm ys xss options)
     ...
     (t/is (v/delta-eq (get-in analysis [:leverage :hat] [##Inf]) (:hat influence)))
     (t/is (v/delta-eq (get-in analysis [:leverage :sigmas] [##Inf]) (:sigma influence)))
     (t/is (v/delta-eq (flatten (get-in analysis [:leverage :coefficients] [##Inf]))
...
(defn glm-tests
  ...
  ([ys xss options glm-r]
   (let [options (merge {:epsilon 1.0e-16 :max-iters 100} options)
     ...
     (t/is (v/delta-eq (get-in glm [:residuals :working] [##Inf]) (:residuals rglmdata) 1.0e-5))
     ...
     (when-not (:no-analysis? options)
       ...
       (t/is (v/delta-eq (flatten (get-in analysis [:leverage :coefficients] [##Inf]))
                         (:coefficients influence)))
       (t/is (v/delta-eq (get-in analysis [:influence :cooks-distance] [##Inf])
                         (-> rglm stats/cooks-distance rr/r->clj) 1.0e-5))
       (t/is (v/delta-eq (get-in analysis [:influence :dffits] [##Inf])
