...
;; ==== Fastmath Vector Audit — Group 2.8: Elementwise math wrappers ====
;; Reference: the underlying scalar functions (fastmath.core) and `fmap` (fastmath.vector,
;; Group 2.2) are both already independently audited/verified elsewhere. What this group
;; specifically verifies is that each `primitive-ops`-generated wrapper is wired to the
;; CORRECT underlying scalar function (a plausible bug shape given ~58 near-identical
;; generated defns: a copy-paste/symbol-list error would silently produce a working
;; function computing the wrong thing). Data-driven: for each [vector-fn core-fn x] triple,
;; asserts (vector-fn [x, 0.7x]) == [core-fn(x), core-fn(0.7x)]; `x` chosen within the
;; mathematical domain of each `core-fn` (confirmed via REPL, 2026-09-23).
...
(t/deftest elementwise-primitive-ops-test
  ...
  (doseq [[vfn cfn ^double x] elementwise-fn-cases]
    (let [y (* 0.7 x)
          expected (sut/vec2 (double (cfn x)) (double (cfn y)))
          actual (vfn (sut/vec2 x y))]
      (t/is (sut/delta-eq expected actual) (str "mismatch for " vfn)))))
