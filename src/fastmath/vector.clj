...
(ns fastmath.vector
  ...
  * Fixed size (custom types):
      ...
      * ArrayVec - fixed size vector as double array, n-dimensional, creator [[array-vec]]
  * Variable size:
  ...
  * `clojure.core.matrix.protocols`
  ...
  (:import [clojure.lang Counted IFn PersistentVector IPersistentVector IPersistentCollection Seqable Sequential Reversible Indexed ILookup]
           [clojure.core Vec]))
...
(deftype ArrayVec [^doubles array]
  ...
  (as-vec [v xs] (ArrayVec. (double-array (take (count v) xs))))
  (as-vec [v] (as-vec v (repeat 0.0)))
  (fmap [_ f] (ArrayVec. (amap array idx ret ^double (f (aget array idx)))))
...
(defn clamp
  "Clamp elements."
  {:metadoc/categories #{:op}}
  ([v ^double mn ^double mx] (fmap v #(m/constrain ^double % mn mx)))
  ([v] (clamp v 0 Double/MAX_VALUE)))
